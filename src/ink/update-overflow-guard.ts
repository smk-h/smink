 ...
 * someone else entirely (Divider measure loops, streaming measure
 * cascades, syntax-highlight diff loops).
 *
...
/** Test seam: forget registered quench actions. */
export function resetOverflowQuenchesForTest(): void {
  quenches.clear()
}

/**
 * Test seam: pull a source's cooldown window back by `ms` so tests can
 * verify the escalating backoff without waiting real time. No-op for
 * unknown sources.
 */
export function advanceQuenchCooldownForTest(source: string, ms: number): void {
  const state = sources.get(source)
  if (state !== undefined) state.quenchUntil -= ms
}
...
export function swallowNestedUpdateOverflow(error: unknown, source: string): boolean {
  ...
  if (state.count >= TRIP_THRESHOLD && now >= state.quenchUntil) {
    state.trips += 1
    const ms = Math.min(
      TRIP_BACKOFF_BASE_MS * 2 ** (state.trips - 1),
      TRIP_BACKOFF_MAX_MS,
    )
 ...
 * unchanged. Idempotent; opt out with SMINK_NO_185_PROCESS_GUARD=1 when a
...
export function installNestedUpdateOverflowProcessGuard(): void {
  ...
  if (process.env.SMINK_NO_185_PROCESS_GUARD === '1') return
