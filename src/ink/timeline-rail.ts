 ...
 * always what you can click (a tick that is drawn but not clickable, or
 * vice versa, is impossible by construction).
 *
 * Viewport semantics (all derived from the same turn-top list):
 *
 ...
 *    row; naming it would make the down chevron repeat itself forever).
 */
...
export interface TimelineTurn {
  /** Row id of the turn (jump/preview target). */
  id: number
  /** Content-space top of the turn's prompt text (scrollTo(target) pins it
   *  to the viewport top). */
  top: number
  /** First non-empty prompt line, char-capped (see clipPreview). */
  preview: string
  /**
   * True while the turn sits BEFORE the fold window (older than the most
   * recent rendered rows): its top is unknown (unmounted since before the
   * fold; `top` carries -1) and clicking the tick must first reveal the
   * folded history rather than scrollTo(-1). Rendering a tick for it is
   * still correct — the turn EXISTS and is navigable, it is just folded
   * away right now.
   */
  folded?: boolean
}

/** The per-commit snapshot consumed by the rail. */
export interface TimelineSnapshot {
  ...
  /** Up target (strictly above the top), null at the first turn. */
  upId: number | null
  /** Down target (below the top and reachable), null at the end. */
  downId: number | null
}
...
export interface RailGeometry {
  ...
  windowStart: number
  windowEnd: number
  /** Row of the up chevron (block-local, within the rail column). */
  upRow: number
  /** Row of the first tick. */
  tickTop: number
  /** Row of the down chevron. */
  downRow: number
}
...
export function computeRailGeometry(
  ...
  // Vertically center the up-chevron + ticks + down-chevron stack.
...
/**
 * First non-empty line, char-capped with an ellipsis marker. Bounded work:
 * the scan stops at the first non-empty line and slices at the cap, so a
 * huge one-line prompt costs O(cap), not O(line length). Char cap (not
 * display width) on purpose — this bounds the stored snapshot; the hover
 * card re-wraps to its own width (wrapPreviewLines).
 */
