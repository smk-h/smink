...
//
// Ported from Claude Code's execFileNoThrow.
// Differences:
// - No getCwd() dependency (useCwd=false or pass cwd explicitly)
// - No logError on catch (smink-specific: let callers decide)
// - No execSyncWithDefaults_DEPRECATED (business logic)
...
export function execFileNoThrow(
  ...
  return execFileNoThrowWithCwd(file, args, {
    ...
    cwd: undefined, // smink: no getCwd() dependency
    env: options.env,
    stdin: options.stdin,
    input: options.input,
  })
...
export function execFileNoThrowWithCwd(
  ...
  return new Promise(resolve => {
    ...
    execa(file, args, {
      ...
      .catch((_error: ExecaError) => {
        // smink: no logError dependency, silently resolve
