 ...
 * initialized. Users often start typing their prompt immediately, but those
 * early keystrokes would otherwise be lost during startup.
 *
 * Usage:
 * 1. Call startCapturingEarlyInput() as early as possible
 * 2. When the TUI is ready, call consumeEarlyInput() to get any buffered text
...
/**
 * Start capturing stdin data early, before the TUI is fully initialized.
 ...
 * Handles control characters, escape sequences, backspace, and printable text.
 */
 ...
 * Removes the 'readable' listener but does not reset stdin state —
 * the REPL's Ink App will manage stdin from here.
 */
